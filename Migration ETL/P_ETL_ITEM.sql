create or replace
PROCEDURE P_ETL_ITEM AS
CURSOR EXT_CUR_ITEM IS
SELECT exitem.*, extstatio.ID as PRODID, mtitem.ID as MTITEMID
FROM T_EXT_ITEM exitem 
LEFT JOIN T_MT_ITEM mtitem
ON exitem.CODE = mtitem.CD
LEFT JOIN T_MT_STATIO extstatio
ON exitem.ITEM_CODE LIKE 'S%' AND EXITEM.ITEM_CODE= extstatio.CD;
EXT_ROW EXT_CUR_ITEM%ROWTYPE;
CURSOR EXT_CUR_BOOK IS
SELECT exitem.*, extbook.ID as BOOKID, mtitem.ID as MTITEMID
FROM T_EXT_ITEM exitem 
LEFT JOIN T_MT_ITEM mtitem
ON exitem.CODE = mtitem.CD
LEFT JOIN T_MT_BOOK extbook
ON exitem.ITEM_CODE LIKE 'B%' AND exitem.ITEM_CODE = extbook.CD;
BEGIN

FOR EXT_ROW IN EXT_CUR_BOOK LOOP 
	IF EXT_ROW.MTITEMID IS NULL THEN
	INSERT INTO T_MT_ITEM
	VALUES
	(
		SYS_GUID(),
		EXT_ROW.BOOKID,
		EXT_ROW.CODE
	);
	ELSE
	UPDATE T_MT_ITEM
	SET PRODUCT_ID = EXT_ROW.BOOKID
		WHERE CD = EXT_ROW.CODE;
	END IF;
END LOOP;        

FOR EXT_ROW IN EXT_CUR_ITEM LOOP  
	IF EXT_ROW.MTITEMID IS NULL THEN
	INSERT INTO T_MT_ITEM
	VALUES
	(
		SYS_GUID(),
		EXT_ROW.PRODID,
		EXT_ROW.CODE
	);
	ELSE
	UPDATE T_MT_ITEM
	SET PRODUCT_ID = EXT_ROW.PRODID
		WHERE CD = EXT_ROW.CODE;
	END IF;
END LOOP;
COMMIT;
END P_ETL_ITEM;
